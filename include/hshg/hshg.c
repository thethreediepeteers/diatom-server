...
hshg_attrib_unused static const _hshg_entity_t _hshg_entity_max =
    max_t(_hshg_entity_t);

hshg_attrib_unused static const _hshg_cell_t _hshg_cell_max =
    max_t(_hshg_cell_t);

hshg_attrib_unused static const _hshg_cell_sq_t _hshg_cell_sq_max =
    max_t(_hshg_cell_sq_t);

#undef max_t

hshg_attrib_const static uint8_t hshg_max_grids(_hshg_cell_t side) {
  uint8_t grids_len = 0;
...
hshg_attrib_const static _hshg_cell_sq_t hshg_max_cells(_hshg_cell_t side) {
  _hshg_cell_sq_t cells_len = 0;

  do {
    const _hshg_cell_sq_t new =
        cells_len + (_hshg_cell_sq_t)side _2D(*side) _3D(*side);
...
_hshg* _hshg_create(const _hshg_cell_t side, const uint32_t size) {
  assert(__builtin_popcount(side) == 1 && "Both arguments must be powers of 2");
  assert(__builtin_popcount(size) == 1 && "Both arguments must be powers of 2");

  const _hshg_cell_sq_t cells_len = hshg_max_cells(side);
  const uint8_t grids_len = hshg_max_grids(side);

  _hshg* const hshg = malloc(sizeof(_hshg) + sizeof(_hshg_grid) * grids_len);
  ...
  for (uint8_t i = 0; i < grids_len; ++i) {
    (void)memcpy(
        ...
        &((_hshg_grid){
            ...
            .cells_side = _side,
            _3D(.cells_sq = (_hshg_cell_sq_t)_side * _side, ).cells_mask =
                _side - 1,

            _2D(.cells2d_log = __builtin_ctz(_side) << 0, )
                _3D(.cells3d_log = __builtin_ctz(_side) << 1, )
    ...
    idx += (_hshg_cell_sq_t)_side _2D(*_side) _3D(*_side);
    _side >>= 1;
    _size <<= 1;
  }
...
void _hshg_free(_hshg* const hshg) {
  free(hshg->entities);
  free(hshg->cells);
  free(hshg);
}

hshg_attrib_const size_t _hshg_memory_usage(const _hshg_cell_t side,
...
int _hshg_set_size(_hshg* const hshg, const _hshg_entity_t size) {
  assert(!hshg->calling &&
...
static void invalidate_entity(_hshg_entity* const entity) {
  entity->cell = _hshg_cell_sq_max;
}

static int invalid_entity(const _hshg_entity* const entity) {
  return entity->cell == _hshg_cell_sq_max;
}

static _hshg_entity_t hshg_get_entity(_hshg* const hshg) {
...
static void hshg_return_entity(_hshg* const hshg) {
...
hshg_attrib_const static _hshg_cell_t
...
hshg_attrib_const static _hshg_cell_sq_t
grid_get_idx(const _hshg_grid* const grid,
             const _hshg_cell_sq_t x _2D(, const _hshg_cell_sq_t y)
                 _3D(, const _hshg_cell_sq_t z)) {
  return x _2D(| (y << grid->cells2d_log)) _3D(| (z << grid->cells3d_log));
}

hshg_attrib_const static _hshg_cell_t idx_get_x(const _hshg_grid* const grid,
...
_2D(
    ...
    hshg_attrib_const static _hshg_cell_t idx_get_y(
        const _hshg_grid* const grid, const _hshg_cell_sq_t cell) {
      return (cell >> grid->cells2d_log) _3D(&grid->cells_mask);
    }
...
_3D(
    ...
    hshg_attrib_const static _hshg_cell_t idx_get_z(
        const _hshg_grid* const grid, const _hshg_cell_sq_t cell) {
...
grid_get_cell(const _hshg_grid* const grid,
  ...
  _2D(const _hshg_cell_t cell_y = grid_get_cell_1d(grid, y);)
  _3D(const _hshg_cell_t cell_z = grid_get_cell_1d(grid, z);)

  return grid_get_idx(grid, cell_x _2D(, cell_y) _3D(, cell_z));
}

hshg_attrib_const static uint8_t hshg_get_grid(const _hshg* const hshg,
                                               const _hshg_pos_t r) {
...
static void hshg_reinsert(_hshg* const hshg, const _hshg_entity_t idx) {
  _hshg_entity* const entity = hshg->entities + idx;
  _hshg_grid* const grid = hshg->grids + entity->grid;

  entity->cell =
      grid_get_cell(grid, entity->x _2D(, entity->y) _3D(, entity->z));
...
int _hshg_insert(_hshg* const hshg,
                 const _hshg_pos_t x _2D(, const _hshg_pos_t y)
                     _3D(, const _hshg_pos_t z),
...
static void hshg_remove_light(_hshg* const hshg) {
...
void _hshg_remove(_hshg* const hshg) {
...
void _hshg_move(_hshg* const hshg) {
  assert(hshg->updating &&
  ...
  const _hshg_cell_sq_t new_cell =
      grid_get_cell(grid, entity->x _2D(, entity->y) _3D(, entity->z));

  if (entity->cell != new_cell) {
    hshg_remove_light(hshg);
    hshg_reinsert(hshg, idx);
  }
}

void _hshg_resize(_hshg* const hshg) {
  assert(hshg->updating &&
...
void _hshg_update(_hshg* const hshg) {
  assert(hshg->update);
...
void _hshg_update_multithread(const _hshg* const hshg, const uint8_t threads,
                              const uint8_t idx) {
...
void _hshg_update_cache(_hshg* const hshg) {
...
void _hshg_collide(_hshg* const hshg) {
  assert(hshg->collide);
  assert(!hshg->calling &&
  ...
  const _hshg_entity* const entity_max = hshg->entities + hshg->entities_used;
...
#define loop_over(from)                                                        \
  ...
  for (entity = hshg->entities + 1; entity != entity_max; ++entity) {
    ...
    _2D(_hshg_cell_t cell_y = idx_get_y(grid, entity->cell);)
    _3D(_hshg_cell_t cell_z = idx_get_z(grid, entity->cell);)
    _3D(if (cell_z != 0) {
      if (cell_y != 0) {
        const _hshg_entity_t* const cell =
            grid->cells + (entity->cell - grid->cells_sq - grid->cells_side);
      ...
      {
        const _hshg_entity_t* const cell =
            grid->cells + (entity->cell - grid->cells_sq);
    ...
    while (grid->shift) {
      cell_x >>= grid->shift;
      _2D(cell_y >>= grid->shift;)
      _3D(cell_z >>= grid->shift;)

      grid += grid->shift;

      const _hshg_cell_t min_cell_x = cell_x != 0 ? cell_x - 1 : 0;

      _2D(const _hshg_cell_t min_cell_y = cell_y != 0 ? cell_y - 1 : 0;)

      _3D(const _hshg_cell_t min_cell_z = cell_z != 0 ? cell_z - 1 : 0;)

      const _hshg_cell_t max_cell_x =
          cell_x != grid->cells_mask ? cell_x + 1 : cell_x;

      _2D(const _hshg_cell_t max_cell_y =
              cell_y != grid->cells_mask ? cell_y + 1 : cell_y;)

      _3D(const _hshg_cell_t max_cell_z =
              cell_z != grid->cells_mask ? cell_z + 1 : cell_z;)

      _hshg_cell_t cur_x;
      _2D(_hshg_cell_t cur_y;)
      _3D(_hshg_cell_t cur_z;)

      _3D(for (cur_z = min_cell_z; cur_z <= max_cell_z; ++cur_z)) {

        _2D(for (cur_y = min_cell_y; cur_y <= max_cell_y; ++cur_y)) {

          for (cur_x = min_cell_x; cur_x <= max_cell_x; ++cur_x) {
            const _hshg_cell_t cell =
                grid_get_idx(grid, cur_x _2D(, cur_y) _3D(, cur_z));
...
int _hshg_optimize(_hshg* const hshg) {
  assert(!hshg->calling &&
...
static void hshg_map_pos(const _hshg* const hshg, _hshg_cell_t* const ret,
  ...
  if (_x1 < 0) {
    const _hshg_pos_t shift =
        (((_hshg_cell_t)(-_x1 * hshg->inverse_grid_size) << 1) + 2) *
        hshg->grid_size;
...
static void hshg_query_common(const _hshg* const hshg,
                              const _hshg_pos_t x1 _2D(, const _hshg_pos_t y1)
                                  _3D(, const _hshg_pos_t z1),
                              const _hshg_pos_t x2 _2D(, const _hshg_pos_t y2)
  ...
  _2D(hshg_map_pos(hshg, &y.start, y1, y2);)
  _3D(hshg_map_pos(hshg, &z.start, z1, z2);)
  ...
  while (1) {
    ...
    _2D(const _hshg_cell_t s_y = y.start != 0 ? y.start - 1 : 0;)
    _3D(const _hshg_cell_t s_z = z.start != 0 ? z.start - 1 : 0;)

    const _hshg_cell_t e_x = x.end != grid->cells_mask ? x.end + 1 : x.end;

    _2D(const _hshg_cell_t e_y = y.end != grid->cells_mask ? y.end + 1 : y.end;)

    _3D(const _hshg_cell_t e_z = z.end != grid->cells_mask ? z.end + 1 : z.end;)

    _3D(for (_hshg_cell_t z = s_z; z <= e_z; ++z)) {

      _2D(for (_hshg_cell_t y = s_y; y <= e_y; ++y)) {

        for (_hshg_cell_t x = s_x; x <= e_x; ++x) {
          _hshg_entity_t j;

          const _hshg_cell_sq_t cell = grid_get_idx(grid, x _2D(, y) _3D(, z));

          for (j = grid->cells[cell]; j != 0;) {
            const _hshg_entity* const entity = hshg->entities + j;

            if ((entity->x + entity->r >= x1 && entity->x - entity->r <= x2)
                    _2D(&&entity->y + entity->r >= y1 &&
                        entity->y - entity->r <= y2)
                        _3D(&&entity->z + entity->r >= z1 &&
    ...
    if (grid->shift) {
      x.start >>= grid->shift;
      _2D(y.start >>= grid->shift;)
      _3D(z.start >>= grid->shift;)

      x.end >>= grid->shift;
      _2D(y.end >>= grid->shift;)
      _3D(z.end >>= grid->shift;)
...
void _hshg_query(_hshg* const hshg,
                 const _hshg_pos_t x1 _2D(, const _hshg_pos_t y1)
                     _3D(, const _hshg_pos_t z1),
                 const _hshg_pos_t x2 _2D(, const _hshg_pos_t y2)
                     _3D(, const _hshg_pos_t z2)) {
...
void _hshg_query_multithread(const _hshg* const hshg,
                             const _hshg_pos_t x1 _2D(, const _hshg_pos_t y1)
                                 _3D(, const _hshg_pos_t z1),
                             const _hshg_pos_t x2 _2D(, const _hshg_pos_t y2)
                                 _3D(, const _hshg_pos_t z2)) {
